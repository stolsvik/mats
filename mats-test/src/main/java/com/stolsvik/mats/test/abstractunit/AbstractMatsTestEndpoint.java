 ...
 * </ul>
 * Set's up a {@link MatsEndpoint} which processor can be modified on the fly. Also provides utility methods
 * to extract incoming requests and verify that endpoint hasn't been invoked.
 *
 * @param <R>
...
public abstract class AbstractMatsTestEndpoint<R, I> {
     ...
     * @return an instance of {@link AbstractMatsTestEndpoint}.
     */
     ...
     */
    public void setProcessLambda(ProcessSingleLambda<R, I> processLambda) {
     ...
     */
    public void before() {
        log.debug("+++ JUnit/Jupiter +++ BEFORE on Rule/Extension '" + id(getClass()) + "'.");

        // ?: Is the mats factory defined?
        if (_matsFactory == null) {
            // -> No, then we can't continue. Throw hard.
            throw new MatsFactoryNotSetException("+++ JUnit/Jupiter +++ '" + id(getClass()) + "': MatsFactory is "
                    + "not set, thus cannot complete setup of test endpoint.\n Provide me a MatsFactory either through "
                    + "setting it explicitly through setMatsFactory(...), or use a Spring TestExecutionListener"
                    + "(RuleMatsAutowireTestExecutionListener||ExtensionMatsAutowireTestExecutionListener) to"
        ...
        // forced by a single stage endpoint.
        ...
        _endpoint.stage(_incomingMsgClass, (ctx, state, msg) -> {
            try {
                ...
                if (_processLambda != null) {
                    // -> Yes, execute it.
                    log.debug("+++ [" + _endpointId + "] executing user defined process lambda, replying with reply"
                            + "class of the type [" + _replyMsgClass + "]");
        ...
        log.debug("+++ JUnit/Jupiter +++ BEFORE done on Rule/Extension '" + id(getClass()) + "'.");
    }
     ...
     * </ul>
     * <p>
     * Note: If a test utilizes {@link AbstractMatsTest#removeAllEndpoints()} in a method annotated with
     * {@link org.junit.Before} inside the test class. Then this is obsolete, however for the purpose of cleanliness
     * the endpoint will clean up after itself.
     */
    public void after() {
        log.debug("+++ JUnit/Jupiter +++ AFTER on Rule/Extension '" + id(getClass()) + "'.");
        ...
        log.debug("+++ JUnit/Jupiter +++ AFTER done on Rule/Extension '" + id(getClass()) + "'.");
    }
    ...
    static class SynchronizedInvocationList<I> {
         ...
         * elements. Given that the list is filled up with the desired number of elements within the specified
         * timeout period, it will return these elements. Should the number of elements not reach the specified count
         * it will timeout and throw an {@link AssertionError}.
         *
         * @param count
    ...
    /**
     * Thrown by {@link #verifyNotInvoked} should the internal {@link #_synchronizedInvocationList} contain any
     * elements indicating an invocation.
     */
