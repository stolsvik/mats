...
public interface MatsSerializer<Z> {
     ...
     *
     * @param traceId       the Trace Id of this new {@link MatsTrace}.
     * @param keepMatsTrace     to which extent the MatsTrace should "keep trace", i.e. whether all Calls and States should be kept
     *                      through the entire flow from initiation to terminator - default shall be {@link KeepMatsTrace#COMPACT}.
     *                      The only reason for why this exists is for debugging: The implementation cannot depend on this
     *                      feature. To see the call history, do a toString() on the ProcessContext of the lambda, which should
     *                      perform a toString() on the corresponding MatsTrace, which should have a human readable trace output.
     * @param nonPersistent whether the message should be JMS-style "non-persistent" - default shall be <code>false</code>, i.e.
     *                      the default is that a message is persistent.
     * @param interactive   whether the message should be prioritized in that a human is actively waiting for the reply, default
     *                      shall be <code>false</code>.
     * @return a new instance of the underlying {@link MatsTrace} implementation.
     */
    MatsTrace<Z> createNewMatsTrace(String traceId, KeepMatsTrace keepMatsTrace, boolean nonPersistent, boolean interactive);

    /**
     * Used for serializing the {@link MatsTrace} to a byte array.
     *
     * @param matsTrace the {@link MatsTrace} instance to serialize.
    ...
    interface SerializedMatsTrace {
        ...
        /**
         * @return the number of bytes the trace became <i>before</i> compression. (The number after compression is
         * just to do '.length' on {@link #getMatsTraceBytes() the bytes}.
         */
     ...
     *
     * @param serialized the byte array from which to reconstitute the {@link MatsTrace}.
    ...
    interface DeserializedMatsTrace<Z> {
         ...
         * (potentially) compressed trace you had when sending in the byte array in {@link #deserializeMatsTrace(byte[], String)}.
         */
     ...
     *
     * @param object the object to serialize. If <code>null</code> is provided, then <code>null</code> shall be returned.
     ...
     *
     * @param serialized the value of type T that should be deserialized into an object of Class T. If <code>null</code> is
     *                   provided, then <code>null</code> shall be returned.
     * @param type       the Class that the supplied value of type Z is thought to represent (i.e. the STO or DTO class).
