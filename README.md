...
In a Mats service consisting of multiple stages, the stages of that service are connected with a *state object*, which gives an effect of having "request scoped variables" that are present through the different stages of the service - hence stateful. This state object is passed along with the envelope of the messages in the flow, instead of residing on the server - hence stateless.
