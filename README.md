...
A Mats service consists of one or multiple stages where the first stage receives an incoming message from an initiator or another service, and the last stage replies with a message to the invoker. A stage of *Service A* may send a request to another *Service B*, where the reply from that service will be received on the subsequent stage of *Service A*. Furthermore, *Service B* may itself also consist of multiple stages which performed requests to *Service C* and *D* - with arbitrary nesting levels possible.

In a Mats service consisting of multiple stages, the stages are connected with a *state object* which is passed along with the message flow, which gives an effect of having "request scoped variables" that are present through the different stages of the service.

The intention is that coding a Mats endpoint *feels like* coding a normal service method that performs synchronous RPC (e.g. REST) calls to other services, while reaping all the benefits of an asynchronous Message-Oriented communication paradigm. In addition, each stage is independently transactional, making a system made up of these services exceptionally resilient against any type of failure.
